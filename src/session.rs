...
use log::trace;
use metrics::{increment_gauge, register_gauge};
...
pub static SESSION_PUBREC_PACKETS_COUNT: &str = "session.pubrec.count";
...
impl Session {
    ...
    pub fn register_pubrec(&mut self, client_id: String, packet: &ControlPacket) {
        ...
        if self.client2pubrec.contains_key(&(client_id.clone(), packet_id)) {
            ...
            increment_gauge!(SESSION_PUBREC_PACKETS_COUNT, 1.0);
        }
    }
...
impl Session {
    pub fn new() -> Self {
        ...
        Session { client2pub_qos0_packets, client2pub_qos1_packets, client2pub_qos2_packets, client2puback, client2pubrel, client2pubrec }
    }
}
